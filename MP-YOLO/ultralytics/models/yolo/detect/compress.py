...
# "magnitude": 使用幅值重要性（Magnitude Importance）进行剪枝。
# "lamp": 使用LAMP（Learned Approximate Matrix Prior）重要性进行剪枝。
# "slim": 使用Structured Learning and Inference Module（SLIM）进行剪枝。
# "group_slim": 使用组稀疏正则化的SLIM进行剪枝。
# "group_norm": 使用组范数（Group Norm）重要性进行剪枝。
# "group_sl": 使用组稀疏正则化的组范数重要性进行剪枝。
# "growing_reg": 使用Growing Regularization进行剪枝。
# "group_hessian": 使用组Hessian矩阵的重要性进行剪枝。
# "group_taylor": 使用组Taylor近似的重要性进行剪枝。
